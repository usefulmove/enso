...
- [ ] `session-start` is updated to read QUEUE.md and dispatch the highest-priority `ready` story
- [ ] `enso-start` falls back to `ls docs/stories/` if QUEUE.md is missing (backward compat)
- [ ] `session-persist` is updated to advance story states in QUEUE.md during closeout
...
- `skills/session-persist/SKILL.md`
...
3. **Update `session-start` skill** — Replace `ls -t docs/stories/*.md` with reading QUEUE.md; dispatch highest-priority `ready` story; fallback to filesystem if QUEUE.md missing.
4. **Update `session-persist` skill** — Add a step to advance story state (`ready → running → done` / `running → blocked`) during closeout.
...
- Session lifecycle skills (`session-start`, `session-persist`, `read-session-logs`, `enso-reference`) live in `skills/` as standard opencode skills. QUEUE.md is project-local. The skills must resolve the project root to find QUEUE.md.
...
- [ ] Load `session-start` skill on this project → confirms it reads QUEUE.md and dispatches the highest-priority ready story
- [ ] Load `session-start` skill on a project without QUEUE.md → falls back to `ls docs/stories/`
- [ ] Load `session-persist` skill with a completed story → confirms the story transitions to `done` in QUEUE.md
