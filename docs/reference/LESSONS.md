...
- [ ] A proposed "v2 schema" can collapse into a no-schema discipline layer under pressure-testing — interrogate whether a change is structural (new fields/states) or behavioral (authoring discipline) before bumping a schema/contract version.
- [ ] OKR value for an agent harness lives in the "outcome, not activity" discipline and binary thresholds — not grading, stretch, or cadence, which map to human performance management and weaken auditable pass/fail loops.
- [ ] A threshold is both measurable and binary: encode magnitude as `metric <= target` and keep baselines in the Goal as motivation, so verification's `pass_when` stays the runnable form of the key result (no duplicate measurement layer).
- [ ] Self-contained ("egg") stories trade automatic PRD traceability for autonomy and unplanned work; recover coherence retrospectively (curator maps completed stories back to goals) rather than with forward pointers — "align, not cascade."
